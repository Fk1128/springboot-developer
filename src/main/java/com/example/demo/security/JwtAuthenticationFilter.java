...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if(token != null && !token.equalsIgnoreCase("null")) {
           ...
           // 인증된 사용자와 그 사용자으 ㅣ권한정보(Authorities)를 담는 역할을 한다 .
           ...
           //request 로부터 인증 세부정보를 생성하는 역할을 한다 .
           
           //.buildDetails(request)) buildDetails 실행결과가 request 객체다 .
           // request 객체에서 인증과 관련된 추가적인 정보를 추출한다 .
           // 사용자의 세션ID, 클라이언트의 IP주소 등의 메타데이터를 포함한다 .
           ...
           //스프링 시큐리티에서 사용자의 인증정보와 Context를 관리하는 중심클래스
           // 애플리케이션 내에서 현재 인증된 사용자의 정보를 저장하고 제공하는 역할을 한다 .      
     
           SecurityContext securityContext = SecurityContextHolder.createEmptyContext();           
           
           //setAuthentication(authentication)
           // 현재 요청에 대한 인증정보를 SecurityContext에 저장하여
           // 스프링 시큐리티가 해당 사용자를 인증된 자용자로 인식하게 해주는 메서드 
           securityContext.setAuthentication(authentication);
           
           // 인증을 완료한후 setContext메서드를 사용해 인증된 사용자정보를 저장할 수 있다 .
